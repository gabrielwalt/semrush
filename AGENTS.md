...
**No `aem.js` edits.** It is the core EDS library. Never touch it.

**Lint always.** Run `npm run lint` after every code change. Auto-fix with `npm run lint:fix`.

**CSS scoping.** Every CSS selector must start with `.{blockname}`. No unprefixed class names that could leak between blocks.

**CSS custom properties.** Use tokens from `styles.css` instead of hardcoded values. Add new tokens when a value recurs.

**No positional assumptions.** Pages can have any combination of blocks in any order.

**Test visually.** After any CSS/JS change, verify at `localhost:3000` via Playwright or `curl`. Encourage the user to open the Console preview panel to see the result.
...
**Variants:** CSS classes on the block element: `<div class="cards featured">` → `.cards.featured {}`.  
