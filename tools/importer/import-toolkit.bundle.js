...
var CustomImportScript = (() => {
  ...
  // tools/importer/import-toolkit.js
  var import_toolkit_exports = {};
  __export(import_toolkit_exports, {
    default: () => import_toolkit_default
  });
  ...
  function parseHero(section, document) {
    ...
    wp1.textContent = input && input.getAttribute("placeholder") || "Enter domain";
    const wp2 = document.createElement("p");
    wp2.textContent = btn && btn.textContent.trim() || "Get started";
    ...
    if (logos.length) {
      ...
      logos.forEach((img) => {
        const p = document.createElement("p");
        p.appendChild(pic(document, img.getAttribute("src"), img.alt));
        strip.appendChild(p);
      });
  ...
  function parseValueProp(section, document) {
    ...
    if (li.length) {
      ...
      li.forEach((x) => {
        ...
        item.textContent = x.textContent.trim();
        ul.appendChild(item);
      });
      cell.appendChild(ul);
    }
  ...
  function parseStatsCta(section, document) {
    ...
    wp2.textContent = btn && btn.textContent.trim() || "Get started";
    wi.append(wp1, wp2);
    widget.appendChild(wi);
    out.appendChild(WebImporter.DOMUtils.createTable([["Insights Widget"], [widget]], document));
    const statCols = [...section.querySelectorAll('[class*="advantages__column"]')].filter((el) => el.querySelector('[class*="advantages__title"]'));
    ...
    statCols.forEach((it) => {
      ...
      if (label) {
        ...
        lp.textContent = label.textContent.trim();
        cell.appendChild(lp);
      }
      cells.push(cell);
    });
  ...
  function parseFaq(section, document) {
    ...
    triggers.forEach((btn) => {
      ...
      qp.textContent = btn.textContent.trim();
      q.appendChild(qp);
      const ans = btn.nextElementSibling || btn.parentElement.querySelector('[class*="content"], [class*="answer"]');
      const a = document.createElement("div");
      if (ans) {
        [...ans.children].forEach((c) => a.appendChild(c.cloneNode(true)));
        if (!a.children.length) {
          const ap = document.createElement("p");
          ap.textContent = ans.textContent.trim();
          a.appendChild(ap);
        }
      }
      rows.push([q, a]);
  ...
  var import_toolkit_default = {
    transform: (payload) => {
      ...
      result.appendChild(WebImporter.DOMUtils.createTable(
        [["Metadata"], ["template", "template-toolkit"]],
        document
      ));
      main.innerHTML = "";
  ...
  return __toCommonJS(import_toolkit_exports);
})();
