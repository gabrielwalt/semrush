...
// Map a page's display name to its content file (path in Notes wins; else heuristic).
function nameToContentPath(name, notes) {
  const inNotes = notes.match(/content\/[\w/-]+\.plain\.html/);
  if (inNotes) return inNotes[0];
  const n = name.toLowerCase();
  if (/homepage/.test(n)) return 'content/index.plain.html';
  if (/semrush one|\bone\b/.test(n)) return 'content/one.plain.html';
  if (/enterprise/.test(n)) return 'content/enterprise/index.plain.html';
...
if (existsSync(statusPath)) {
  ...
  for (const line of tableEnd.split('\n')) {
    ...
    pages.push({
      name,
      url: url === '—' ? null : url,
      file: nameToContentPath(name, notes || ''),
      contentValidated,
      styleValidated,
      unfrozen,
