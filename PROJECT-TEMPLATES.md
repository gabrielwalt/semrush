...
## Per-URL mapping (all 169 in-scope pages)

Every scoped page is mapped to its canonical chrome → template → sub-category. The full machine-readable list is **`catalog/page-template-map.json`** (one row per URL: `{url, raw, chrome, template, sub}`), generated by joining the discovery assignments (`catalog/template-catalog.json`) with the canonical map above. Zero pages unmapped.

Counts by canonical template:

| Chrome | Template (sub-category) | Pages |
|--------|-------------------------|------:|
| app-shell | `app-detail` | 50 |
| careers | `careers-office-detail` | 23 |
| marketing | `marketing-landing:dark` (enterprise) | 20 |
| app-shell | `app-listing:category` | 20 |
| marketing | `tool-detail:single` | 19 |
| marketing | `case-study-detail:detail` | 8 |
| careers | `careers-content` | 4 |
| marketing | `comparison` | 4 |
| marketing | `resource-detail:gated` | 3 |
| careers | `careers-landing` | 2 |
| careers | `careers-text` | 2 |
| marketing | `article:standard` | 2 |
| marketing | `marketing-landing` / `:oneoff-homepage` | 1 + 1 |
| marketing | `tool-detail:hub` · `article:index` · `article:with-video` · `case-study-detail:index` · `resource-detail:index` | 1 each |
| app-shell | `app-listing:storefront-home` | 1 |
| DEFER | `enterprise-demo-ungated` · `product-sample-page` | 1 each |

**Totals by chrome:** marketing 65 (+2 deferred), app-shell 71, careers 31 = **169**. Regenerate with the join script when the catalog changes.
