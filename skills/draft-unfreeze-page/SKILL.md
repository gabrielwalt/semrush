...
See also: `regression-guard` (catches a shared tool shifting under a frozen page), `styling-additively` (the additive discipline that avoids unfreezing in the first place), `draft-validation-gates` (the GATE 2 re-validation that gates re-freezing), `quality-tooling` (`project-state.mjs` `frozen[]`)
