...
| **One-off (any of the above)** | base prefix + `oneoff-<context>` | as per its kind | `teaser (teaser-oneoff-enterprise)`, `section-oneoff-ai-visibility` |
...
- **Page templates** use `template-` in the metadata value; EDS applies that value verbatim as the body class (`body.template-<page-type>`). Note: existing project pages use bare body classes (`homepage`, `semrush-one`, `enterprise`) — new templates should adopt the `template-` prefix; the existing three are grandfathered until refactored.
- **One-offs may embed context in the name** (that's their whole point) — e.g. `section-oneoff-ai-visibility`, `teaser-oneoff-enterprise`. They always carry the `oneoff-` token in addition to their kind's prefix, plus a registry row in `PROJECT-BLOCKS.md`. Context-embedding names are *only* acceptable on one-offs; reusable variants/styles must stay generic.
