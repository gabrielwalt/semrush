...
## Spacing & Layout
- **The Systematic-Tokenization Rule. [auto]** Every `font-size`, `border-radius`, single-value `padding`/`margin`/`gap`, `letter-spacing`, `color`, and `transition` duration uses a **token**, not a raw literal — the whole point of a style system is reusing the same values. Two directives: **(1) Snap near-matches** — a literal that *nearly* equals a token (e.g. `6px`→`--radius-s` 5px, `13px`→`--font-size-body-s` 14px, `46px`→`--font-size-heading-l` 48px) uses the token. **(2) Cover the gap** — a value that *recurs* with no token (used ≥2–3× for the same role) earns a NEW token (e.g. card titles 21px→`--font-size-heading-s`, the 12px small gap→`--space-xxs`); add it to `:root` + PROJECT-DESIGN, don't scatter the literal. Genuine one-offs (a bespoke 180px stat number, a 2px hairline) stay literal. **Responsive-token hazard:** only swap a *fixed* literal for a token whose value doesn't shrink under `@media` — see `typography-craft` and the [auto] note below. The `tools/quality/detect.mjs` `craft-token-literal` matcher flags single-value font-size/spacing literals that equal a FIXED token. <!-- rule:craft-token-literal -->
