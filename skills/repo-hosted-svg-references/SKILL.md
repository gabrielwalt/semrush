...
## Pitfalls
- **Never run an SVG through `createOptimizedPicture`** — the image service can't rasterize SVG to WebP, so the generated `<source>` breaks (img shows `naturalWidth: 0`). Any block that optimizes images must skip `/\.svg(\?|$)/` sources (see `blocks/cards-icon/cards-icon.js`).
