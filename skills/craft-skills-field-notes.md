...
### impeccable coverage audit + 2 imports (2026-06-18)
**Outcome: audited all 28 impeccable `reference/*.md` against our library; ported the 2 with genuine EDS value (interaction-design, animate), captured a tiny perf guardrail, and recorded why the rest stay out.**

- **Coverage map.** Already covered (Phases G/HP/native): typeset→`typography-craft`, colorize→`color-craft`, layout→`layout-craft`, adapt+harden(overflow)→`responsive-adaptation`, brand/product identity→`craft-floor` Identity-Preservation, audit+critique→`craft-floor`+`detect.mjs`+native `excat-visual-critique`, extract+document→our tokenization system+PROJECT-DESIGN, distill→`craft-floor` Distill, polish→`craft-floor` State/Motion, init→`migration-orientation`, live→`block-visual-iteration`+Playwright.
- **Imported `motion-craft`** (from animate.md) — the missing 4th member of the craft family (typography/color/layout/**motion**). Positive method; defers thresholds to `craft-floor`'s 3 craft-motion-* rules. Key EDS-framed content: motion budget (feedback/state-change/one-entrance), the **fade-rise-on-every-section AI tell** as match-and-refuse, duration-by-role table, animate transform/opacity not layout, IntersectionObserver-once.
- **Imported `interaction-states-eds`** (from interaction-design.md) — the **8 interactive states** (not hover-only), `:focus-visible` never bare `outline:none`, **placeholder-is-not-a-label** (real WCAG 4.1.2 — we already hit this once on insights-widget search), and the **dropdown overflow-clipping bug** (absolute overlay clipped by ancestor `overflow:hidden` → fix the overflow chain / lift to fixed/popover, not z-index). Directly relevant to our mega-menu.
- **[user-steered — perf]** optimize.md is React/bundle/CWV/Lighthouse — **deliberately NOT imported as a skill** (EDS best practices already deliver fast loads; Phase HP dropped CWV for the same reason). Captured only the 3 author-breakable guardrails (don't lazy-load above-fold LCP, reserve space for CLS, transform/opacity-only animation) as a short note in `eds-code-conventions`. User confirmed this scope.
- **[deliberately skipped — greenfield-invention verbs]** shape, onboard, clarify(UX copy), delight, overdrive, bolder, quieter, codex, hooks — these invent/amplify UX that a *faithful brand migration* must not impose. They fight Identity-Preservation. Not ported.


### Wire excat-visual-critique v2 in aggressively (2026-06-18)
**Outcome: promoted the native `excat-visual-critique` from a passive "complementary final check" to the LEAD discovery step of every styling pass.**

- **Why:** EMA shipped Visual Critique v2 — unified *extraction-based* design validation (deterministic CSS/structural/content/interaction diffs + weighted similarity %), with **site-mode parallel sub-agents per template**. It's far more thorough than a hand-built diff and should be triggered aggressively, not held for the end.
- **Division of labor (the key framing):** critique **discovers + scores** the gaps (Block/Section/Page/Site modes; never writes CSS) → `block-visual-iteration` measure-first loop **closes** each delta. Critique = discovery engine; our loops = the fix.
- **Wired into 4 places so styling always opens with a critique:** `block-visual-iteration` (new **Step 0 — run critique FIRST**, with a mode-by-target table), `eds-migration-process` GATE 2 (critique is now the gate *opener*, "don't wait to be asked"), `styling-additively` (critique itemizes content-vs-styling deltas before deciding what's additive; re-run on shared frozen pages as a fast regression signal), and the README native-skills section (intro + row rewritten from "complementary" → "trigger aggressively, our delta-discovery engine").
- **General lesson:** when EMA ships a stronger OOTB skill that overlaps a project workflow, don't just footnote it — make it the *entry point* of the matching project skill and state the division of labor explicitly, so it actually fires.


### Extend the animation layer per-page, re-audit selectors (2026-06-18)
**Outcome: ran the enriched detectors on One+Enterprise (clean — 5 accepted brand-gradient warns only) and extended `scroll-animations.js` to cover Enterprise's case-study proof-point stats.**

- **The global scroll-animation layer is selector-driven, so each new page needs a quick "what does it already catch vs miss?" pass.** A DOM probe (count `.reveal-ready`, the stat/card containers, `.case-study-stat-number`, etc.) on each page reveals coverage gaps fast. One + Enterprise were ~90% covered by the homepage selectors (shared blocks: carousel, cards-icon, cards-awards, columns-stats); the only gap was Enterprise's `case-study-stats-grid` / `.case-study-stat-number`.
- **`countUp`'s prefix/suffix regex (`^([^\d.]*)([\d.,]+)(.*)$`) already handles mixed figures** like `4hrs` (num `4`, suffix `hrs`) and `124%` — no special-casing needed; the digit-start guard (`/^[\d]/`) is what keeps it off worded headings.
- **Restraint held:** did NOT add section-wide reveals to the full-width teaser content rows (that's the fade-rise-on-every-section AI tell — motion-craft). Reveals stay on genuine multi-item grids (cards, carousels, the 3-up case-study stats).
- **Reduced-motion stayed safe across both pages** without any per-page work — the `if (REDUCED) return` guard + JS-only `.reveal-ready`/`.bar-ready` classes mean new selectors inherit the safety automatically. Confirmed: 0 animation classes + final stat values under emulated reduce on One AND Enterprise.
