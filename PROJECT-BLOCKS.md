...
| `accordion` | — | Toolkit pages (seo, content, pricing, etc.) | FAQ disclosure list. Row model: question cell + answer cell. Accessible button + aria-expanded + chevron |
| `carousel` | `carousel-expansible`, `carousel-testimonials` | Homepage (resources, solutions), Enterprise (resources + testimonials) | Single horizontal scrolling row of cards with ‹/› nav. Default = image+text resource cards; `carousel-expansible` = product cards that expand; **`carousel-testimonials`** = portrait (4:5, 318px) image-only customer cards (enterprise "How leaders…" — replaced the bespoke `testimonials-carousel` here). |
| `cards-icon` | `cards-icon-feature` | Semrush One, toolkit pages | Icon/image + title + description grid (3-col). Also used for toolkit 3-step + feature-row sections. **`cards-icon-feature`** variant: the leading image is a full product SCREENSHOT (not a 48px icon) — shown full-width + rounded at the card top, with a fixed `aspect-ratio: 3/2` so the card reserves height even before the remote SVG loads (avoids 0-height collapse on mobile / 404 on local dev). Emitted by `import-toolkit.js parseFeatures` (the "infographics" feature rows); the 3-step `parseSteps` stays bare (small icons) |
| `columns-stats` | — | Semrush One, toolkit stats | Large stat number columns (one row, h3 number + label). Toolkit "how many" stats |
...
| `template-toolkit` | `body.template-toolkit` | `template: template-toolkit` metadata (emitted by toolkit import scripts) | Product/toolkit overview pages (seo, content, pricing, local, social, pr, company). Section-heading sizing, centered stats/FAQ, feature-card grid rhythm. Reuses cards-icon, accordion, columns-stats, teaser |
