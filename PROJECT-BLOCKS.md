...
| `section-violet` | Brand-violet background band (`--accent-color` — the one-purple signal), dark ink, centered; closing-CTA primary button inverts to dark fill for contrast | Closing CTA bands on a violet band (e.g. comparison "Get Started with Semrush") | Background — mutually exclusive with other bg styles |
...
| `cards-icon` | `cards-icon-feature`, `cards-icon-tools`, `cards-icon-related` | Semrush One, Feature pages | Icon + title + description feature grid (3-col). **`cards-icon-feature`**: leading image is a full product SCREENSHOT (full-width, rounded, `aspect-ratio: 3/2`); unused by a keeper, parked. **`cards-icon-tools`** (feature pages): bordered white product card — 96px glyph, title, desc, DUAL CTA (primary "Try It Free" soft-neutral fill + secondary "Learn more" quiet text link, pushed to card bottom via `margin-top:auto`); hover-lift. **`cards-icon-related`** (feature pages): bordered link card — title, desc, "N tools" caption, accent "Learn more" link; no icon. All variants scoped by class so the frozen pages + /one are untouched. CSS: `cards-icon.css` |
| `cards-awards` | — | Semrush One | Award badge display for the dark CTA section |
| `comparison-table` | — | Comparison (`/vs/`) | **NEW.** Spec matrix: feature col (muted, left) + Semrush col (highlighted full-height in `--accent-color` — the signature) + competitor col. The header row's first cell holds the feature-column label (e.g. "AI optimization") + "Semrush" + competitor — NO standalone group band above it. Group-band rows (bold ink label) keep a purple spacer so the highlight stays continuous. `Yes`/`No` → CSS-drawn ✓ (ink) / ✗ (`--color-critical`); other text (counts/prices/partial-support) passes through. Row rules are **black/translucent** (`rgb(0 0 0 / 12%)`) so they stay visible over the page gradient; a 1px rounded frame (`--radius-l`, `overflow:hidden`) wraps the matrix. Block JS detects group rows (1 cell, or 3 cells with empty values), the header row (middle cell = "Semrush"), and feature rows. CSS: `comparison-table.css` |
| `reviews` | — | Comparison (`/vs/`) | **NEW.** Single-testimonial carousel ("Here's why marketers love Semrush") — one centered review at a time with prev/next arrow nav + "N / total" counter. Each authored row = one review (blockquote + avatar + name `strong` + role/company). `reviews.js` shows one slide, hides the rest, wires nav. CSS: `reviews.css` |
| `ratings` | — | Comparison (`/vs/`) | **NEW.** Review-platform scores band ("Users Give Semrush Top Ratings") — 3 centered columns (G2 / Capterra / Gartner), each: brand logo + star-rating image + "X,Y out of 5" score. Stacks on <1024px. `ratings.js` tags logo/stars/score. CSS: `ratings.css` |
| `showcase` | — | Comparison (`/vs/`) | **NEW.** Large frameless image + text rows ("See What You Can Do with Semrush") that alternate image side (odd=image right, even=image left via `order`). Each authored row = one item (screenshot image, h3 title, body paragraphs, secondary CTA). Images ~full-column, `--radius-m`, NO glass frame. Stacks image-above-text on mobile. CSS: `showcase.css` |
